...
import com.cocoa.mvideoplayer.VideoManager;
import com.cocoa.mvideoplayer.listener.MediaPlayerListener;
import com.cocoa.mvideoplayer.listener.VideoAllCallBack;
...
import com.cocoa.mvideoplayer.utils.NetInfoModule;
...
import static com.cocoa.mvideoplayer.utils.CommonUtil.getTextSpeed;
...
public abstract class VideoView extends TextureRenderView implements MediaPlayerListener {
    ...
    public boolean setUp(String url, boolean cacheWithPlay, File cachePath, String title) {
        ...
        if (cacheWithPlay && url.startsWith("http") && !url.contains("127.0.0.1") && !url.contains(".m3u8")) {
            HttpProxyCacheServer proxy = VideoManager.getProxy(getActivityContext().getApplicationContext(), cachePath);
            //此处转换了url，然后再赋值给mUrl。
        ...
        if (VideoManager.instance().getMediaPlayer() != null
            ...
            if (speed > 0) {
                ...
                if (mSoundTouch) {
                    ...
                    List<VideoOptionModel> list = VideoManager.instance().getOptionModelList();
                    if (list != null) {
                    ...
                    VideoManager.instance().setOptionModelList(list);
                }
            }
        }
    }

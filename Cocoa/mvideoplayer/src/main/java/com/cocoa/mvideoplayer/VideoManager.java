...
import com.cocoa.mvideoplayer.listener.MediaPlayerListener;
import com.cocoa.mvideoplayer.model.Model;
import com.cocoa.mvideoplayer.model.VideoOptionModel;
import com.cocoa.mvideoplayer.utils.CommonUtil;
import com.cocoa.mvideoplayer.utils.Debuger;
import com.cocoa.mvideoplayer.utils.FileUtils;
import com.cocoa.mvideoplayer.utils.StorageUtils;
import com.cocoa.mvideoplayer.utils.VideoType;
...
public class VideoManager implements IMediaPlayer.OnPreparedListener, IMediaPlayer.OnCompletionListener,
    ...
    private void initVideo(Message msg) {
        try {
            ...
            if (videoType == VideoType.IJKPLAYER) {
                initIJKPlayer(msg);
            } else if (videoType == VideoType.IJKEXOPLAYER) {
                initEXOPlayer(msg);
            }
    ...
    private void initIJKPlayer(Message msg) {
        ...
        try {
            ...
            ((IjkMediaPlayer) mediaPlayer).setDataSource(((Model) msg.obj).getUrl(), ((Model) msg.obj).getMapHeadData());
            mediaPlayer.setLooping(((Model) msg.obj).isLooping());
            if (((Model) msg.obj).getSpeed() != 1 && ((Model) msg.obj).getSpeed() > 0) {
                ((IjkMediaPlayer) mediaPlayer).setSpeed(((Model) msg.obj).getSpeed());
            }
    ...
    private void initEXOPlayer(Message msg) {
        ...
        mediaPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
        try {
            mediaPlayer.setDataSource(context, Uri.parse(((Model) msg.obj).getUrl()), ((Model) msg.obj).getMapHeadData());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void prepare(final String url, final Map<String, String> mapHeadData, boolean loop, float speed) {
        ...
        Model fb = new Model(url, mapHeadData, loop, speed);
        msg.obj = fb;
    ...
    /**
     * 设置了视频的播放类型
     * VideoType IJKPLAYER = 0 or IJKEXOPLAYER = 1;
     */

...
public class SignTeacherJoinHttp extends BaseHttp {
	private Handler handler;
	private int what;
	private boolean isShowProgress;
	public SignTeacherJoinHttp(Context context, Handler handler, boolean isShowProgress, int what) {
	...
	void getData(Handler handler, BaseDataResp responseObj) {
		sendHandler(handler,new Gson().fromJson(responseObj.getSuccess(),SignTeacherJoinResp.class),what);
	}

	@Override
	String getPrimaryUrl() {
		return API.TEACHER_CONTENT;
	}

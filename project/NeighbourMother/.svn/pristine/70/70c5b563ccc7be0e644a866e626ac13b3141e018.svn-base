 ...
 * date: 2016/9/20.
 */
public class CommissionerApplyNurseActivity extends BaseActivity implements View.OnClickListener,SureListener
{
        ...
        switch (msg.what){
            case 1:
                ...
                if (imgs != null){
                    switch (imgs.length){
                        case 1:
                            req.setPic_environment_1(imgs[0]);
                            break;
                        case 2:
                            req.setPic_environment_1(imgs[0]);
                            req.setPic_environment_2(imgs[1]);
                            break;
                        case 3:
                            req.setPic_environment_1(imgs[0]);
                            req.setPic_environment_2(imgs[1]);
                            req.setPic_environment_3(imgs[2]);
                            break;
                    }
                }
                subHttp();
    ...
    case R.id.back:
        ...
        case R.id.btn_finish:
            ...
            if(upFileObjs!=null&&upFileObjs.size()==0){
                req.setPic_environment_1(resp.getPic_environment_1());
                req.setPic_environment_2(resp.getPic_environment_2());
                req.setPic_environment_3(resp.getPic_environment_3());
                subHttp();
    ...
    private void initCell() {
        ...
        cacuCellWH(iv_img1, red_width, red_width, 0, 0, (int) QLScreenUtil.dpToPx(this, unit), 0);
        cacuCellWH(iv_img2, red_width, red_width, (int) QLScreenUtil.dpToPx(this, unit), 0, 0, 0);
        cacuCellWH(iv_img3, red_width, red_width, 0, 0, (int) QLScreenUtil.dpToPx(this, unit), 0);
    }
...
commisInfoSubSuccessDialog.show(getSupportFragmentManager(),"commisInfoSubSuccessDialog");
    ...
    private void setData(ApplyAttacheQuResp applyAttacheQuResp){
        ...
        AuthObj authObj2 = authUtil.get(2);

        authObj0.setNetImgPath(applyAttacheQuResp.getPic_environment_1());
        authObj1.setNetImgPath(applyAttacheQuResp.getPic_environment_2());
        authObj2.setNetImgPath(applyAttacheQuResp.getPic_environment_3());

        authObj0.setHaveImg(TextUtils.isEmpty(applyAttacheQuResp.getPic_environment_1()) ? 0 : 1);
        authObj1.setHaveImg(TextUtils.isEmpty(applyAttacheQuResp.getPic_environment_2()) ? 0 : 1);
        authObj2.setHaveImg(TextUtils.isEmpty(applyAttacheQuResp.getPic_environment_3()) ? 0 : 1);
        authUtil.initData();
    ...
    private ArrayList<UpFileObj> getUpFiles() {
        ...
        if (!TextUtils.isEmpty(authObj0.getNetImgPath()) || !TextUtils.isEmpty(authObj0.getLocImgPath())
                &&!TextUtils.isEmpty(authObj1.getNetImgPath()) || !TextUtils.isEmpty(authObj1.getLocImgPath())
                &&!TextUtils.isEmpty(authObj2.getNetImgPath()) || !TextUtils.isEmpty(authObj2.getLocImgPath())) {
        ...
        if (!TextUtils.isEmpty(authObj1.getLocImgPath())) {
            ...
            if (!TextUtils.isEmpty(authObj2.getLocImgPath())) {
                upFileObjs.add(new UpFileObj(API.FILE_KEY, new File(authObj2.getLocImgPath())));
            }
    ...
    private void subHttp(){
        new KhAttacheApplySubHttp(this,handler,req,2);
    }

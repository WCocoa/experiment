...
public class DiscussionGroupDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    private DiscussionGroupHomeResp                discussionGroupHomeResp;
    ...
    protected void updateUI(Message msg) {
        switch (msg.what) {
            case 1:
                discussionGroupHomeResp = (DiscussionGroupHomeResp) msg.obj;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.back:
                ...
                break;
            case R.id.applydisc:
                if (discussionGroupHomeResp.getGroup().getJoined() == 1) {
                    ...
                    ToastUtils.toastshort(this,"加入讨论组之后才能发帖！");
                }
                break;
            case R.id.join:
                new JoinGroupHttp(this, handler, new JoinGroupReq(discussionGroupResp.getGroup_id()), 3);
                break;
        }
    }
    ...
    private void setHeadData(DiscussionGroupHomeResp discussionGroupHomeResp) {
        display(img, APIConfig.BASE_IMG_URL + discussionGroupHomeResp.getGroup().getGroup_pic(), R.mipmap.default_img);
    ...
    private void setJoin(int isJoin) {
        ...
        if (isJoin == 0) {
            ...
            discussionGroupHomeResp.getGroup().setJoined(0);
            join.setTextColor(getResources().getColor(R.color.white));
        } else {
            join.setText("已加入");
            join.setSelected(true);
            discussionGroupHomeResp.getGroup().setJoined(1);

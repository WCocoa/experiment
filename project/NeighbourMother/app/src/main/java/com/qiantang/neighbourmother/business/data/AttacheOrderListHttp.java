...
public class AttacheOrderListHttp extends BaseHttp {
    ...
    public AttacheOrderListHttp(Context context, Handler handler,int count, int status,int what, boolean isShowProgress) {
    ...
    void getData(Handler handler, BaseDataResp responseObj) {
        List<AttacheOrderListResp> attacheListResps = new Gson().fromJson(responseObj.getSuccess(), new TypeToken<List<AttacheOrderListResp>>() {
        }.getType());
    ...
    String getPrimaryUrl() {
        return API.ATTACHE_ORDER_LIST + "?status=" + status + "&count=" + API.COUNT + "&offset=" + count;
    }
}

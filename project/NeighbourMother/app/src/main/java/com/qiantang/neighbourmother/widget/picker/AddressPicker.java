...
public class AddressPicker extends LinkagePicker {
     ...
     * 省市县数据
     */
    private List<PProvinceObj> provinceList = new ArrayList<PProvinceObj>();

    public AddressPicker(Activity activity, ArrayList<PProvinceObj> data) {
        super(activity);
        parseData(data);
    }

    private void parseData(ArrayList<PProvinceObj> data) {
        int provinceSize = data.size();
        provinceList.clear();
        provinceList.addAll(data);
        //添加省
        for (int x = 0; x < provinceSize; x++) {
            PProvinceObj pro = data.get(x);
            firstList.add(pro.getAreaName());
            ArrayList<PCityObj> cities = pro.getCities();
            ArrayList<String> xCities = new ArrayList<String>();
            ArrayList<ArrayList<String>> xCounties = new ArrayList<ArrayList<String>>();
            int citySize = cities.size();
            //添加地市
            for (int y = 0; y < citySize; y++) {
                PCityObj cit = cities.get(y);
                xCities.add(cit.getAreaName());
                ArrayList<PDistrictObj> counties = cit.getCounties();
    ...
    public void onSubmit() {
        if (onAddressPickListener != null) {
            PProvinceObj province = provinceList.get(selectedFirstIndex);
            PCityObj city = provinceList.get(selectedFirstIndex).getCities().get(selectedSecondIndex);
            PDistrictObj county = null;
            if (!hideCounty) {
    ...
    public interface OnAddressPickListener {
         ...
         */
        void onAddressPicked(PProvinceObj province, PCityObj city, PDistrictObj county);
